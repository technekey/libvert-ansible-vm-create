...
__metaclass__ = type
###
## This file is developed by technekey.com
## Consider giving your comments on the website.(no login needed)
###
            ...
            for task_per_host in task_data:
                ...
                if len(task_per_host[0]) > 70:
                    ...
                    self._display.display(f"{time.strftime('%H:%M:%S', time.localtime(task_per_host[3]))} {task_per_host[0]:<70.70} {task_per_host[1]:>30} {task_per_host[2]:>10.2f}s",
                        color=SUMMARY_COLOR)
        end_time = time.time()
        self._display.display(f"Total Playbook execution time: {end_time - self.playbook_start_time} sec")
